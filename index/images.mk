...
galleries: image_search_json
	psql $(DATABASE) -v ON_ERROR_STOP=1 <<-SQL
		...
		insert into temp_images.image_search_json (image_id, object) select gi.image_id, jsonb_build_object('gallery_ids', jsonb_agg(gi.gallery_id)) from gallery_interactions gi group by image_id;
		insert into temp_images.image_search_json (image_id, object) select gi.image_id, jsonb_build_object('gallery_positions', jsonb_object_agg(gi.gallery_id, gi.position)) from gallery_interactions gi group by image_id;
	SQL
